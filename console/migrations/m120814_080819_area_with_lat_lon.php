...
class m120814_080819_area_with_lat_lon extends Migration
{
    public function up()
    {
        $tableOptions = null;
        ...
        Yii::$app->db->createCommand("DROP TABLE IF EXISTS {{%area}}")->execute();
        $this->createTable('{{%area}}', [
            'id' => $this->integer()->comment('ID'),
            'pid' => $this->integer()->notNull()->defaultValue(0)->comment('父ID'),
            'short_name' => $this->string(32)->comment('简称'),
            'name' => $this->string(64)->comment('名称'),
            'merge_name' => $this->string(128)->comment('全称'),
            'level' => $this->tinyInteger()->notNull()->defaultValue(0)->comment('层级 1 2 3 省市区县'),
            'pinyin' => $this->string(64)->comment('拼音'),
            'code' => $this->string(16)->comment('长途区号'),
            'zip' => $this->string(16)->comment('邮编'),
            'first' => $this->string(8)->comment('首字母'),
            'lat' => $this->decimal(16, 6)->comment('纬度'),
            'lon' => $this->decimal(16, 6)->comment('经度'),
        ], $tableOptions);
        $this->addPrimaryKey('id', '{{%area}}', ['id']);
        $this->createIndex('pid', '{{%area}}', ['pid']);

        $sql = <<<EOD
        INSERT INTO {{%area}} (`id`, `pid`, `short_name`, `name`, `merge_name`, `level`, `pinyin`, `code`, `zip`, `first`, `lon`, `lat`) VALUES
