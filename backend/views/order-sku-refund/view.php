...
$this->title = $model->id;
$this->params['breadcrumbs'][] = ['label' => '退货', 'url' => ['index']];
...
<?php echo $form->field($model, 'refund_amount')->textInput(['maxlength' => true])->label('确认退货金额') ?>
...
<?php //echo $form->field($model, 'express_code')->textInput(['maxlength' => true]) ?>
    ...
    <div class="form-group">
        <?php //echo Html::submitButton('更新', ['class' => 'btn btn-warning', 'data-confirm' => '确认退货?']) ?>
        <?php echo Html::submitInput('同意退货', ['name' => 'accept', 'class' => 'btn btn-danger', 'data-confirm' => '同意退货?']) ?>
        <?php echo Html::submitInput('拒绝退货', ['name' => 'refuse', 'class' => 'btn btn-primary', 'data-confirm' => '拒绝退货?']) ?>
    </div>

<?php ActiveForm::end(); ?>
<?php endif; ?>

<?php if ($model->status == OrderSkuRefund::STATUS_ACCEPT): ?>
    <?php $form = ActiveForm::begin(); ?>
    <?php echo $form->field($model, 'shipping_memo')->textInput(['maxlength' => true]) ?>

    <div class="form-group">
        <?php echo Html::submitInput('确认收货', ['name' => 'confirm', 'class' => 'btn btn-warning', 'data-confirm' => '确认收货?']) ?>
    </div>
